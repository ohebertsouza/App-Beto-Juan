...
// ============================================================================
// ⚠️ ÁREA DE CONFIGURAÇÃO - O ERRO GERALMENTE ESTÁ AQUI ⚠️
// Você precisa ir no Console do Firebase > Configurações do Projeto
// Copiar os dados do "SDK setup and configuration" e colar abaixo.
// ============================================================================
...
// --- Validação de Segurança (Para não travar a tela) ---
    ...
    <div className="h-screen bg-red-50 flex flex-col items-center justify-center p-6 text-center font-sans">
      <div className="bg-white p-6 rounded-2xl shadow-xl max-w-md border-2 border-red-100">
        ...
        <p className="text-gray-600 text-sm mb-4">
          O app está travando porque as chaves de segurança ainda são as de exemplo ("SUA_API_KEY_AQUI").
        </p>
        <div className="bg-gray-100 p-3 rounded text-left text-xs text-gray-700 mb-4 font-mono overflow-x-auto">
          1. Vá em src/App.jsx<br/>
          2. Procure por "const firebaseConfig"<br/>
          3. Cole os dados do seu projeto no Firebase.
        </div>
        <p className="text-xs text-gray-400">
          *Se já configurou, verifique se salvou o arquivo.
        </p>
      </div>
    </div>
  );
...
export default function App() {
  // Se não configurou, para tudo e mostra o aviso
    ...
    if (this.state.hasError) {
      return (
        <div className="p-6 bg-red-50 h-screen flex flex-col items-center justify-center text-center">
          ...
          <h1 className="text-lg font-bold text-red-800">Erro no Aplicativo</h1>
          <pre className="mt-4 bg-white p-4 rounded text-xs text-red-500 text-left max-w-xs overflow-auto">
            {this.state.error?.toString()}
          </pre>
          <button onClick={() => window.location.reload()} className="mt-6 bg-red-500 text-white px-6 py-2 rounded-lg font-bold">Tentar Novamente</button>
        </div>
      );
    }
...
function AppContent() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        // Tenta autenticar anonimamente (precisa estar ativado no console do Firebase)
    ...
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      setUser(currentUser);
      // Recupera sessão local se existir
      const savedProfile = localStorage.getItem('nosdois_profile_v10');
      const savedAuth = localStorage.getItem('nosdois_auth_v10');
    ...
    if (user && profileName) {
        const fetchProfile = async () => {
            try {
                ...
                // Se der erro de permissão, é provável que as regras do Firestore estejam bloqueadas
                if(err.code === 'permission-denied') alert("Erro de Permissão: Verifique as regras do Firestore (Modo de Teste).");
            }
        };
        fetchProfile();
    }
  ...
  const handleAuthSuccess = (name) => {
    setProfileName(name);
    setIsAuthenticated(true);
    localStorage.setItem('nosdois_profile_v10', name);
    localStorage.setItem('nosdois_auth_v10', 'true');
  };
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('nosdois_profile_v10');
    localStorage.removeItem('nosdois_auth_v10');
  };
  ...
  if (authError) {
    return (
        <div className="h-screen flex flex-col items-center justify-center p-6 text-center">
            ...
            <h2 className="font-bold text-red-600">Erro de Conexão</h2>
            <p className="text-sm text-gray-600 mt-2">Não foi possível conectar ao Firebase.</p>
            <p className="text-xs bg-gray-100 p-2 mt-2 rounded font-mono text-red-500">{authError}</p>
            <p className="text-xs text-gray-400 mt-4">Dica: Ative a "Autenticação Anônima" no Console do Firebase.</p>
        </div>
    );
  }
    ...
    return (
        <div className="flex flex-col items-center justify-center h-screen bg-rose-50 p-6">
            <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-sm text-center">
                ...
                <p className="text-sm text-gray-500 mb-6">{mode === 'login' ? 'Entre na sua conta' : 'Crie seu perfil'}</p>
                <form onSubmit={handleSubmit} className="space-y-4 text-left">
                    ...
                    <button type="submit" disabled={loading} className="w-full bg-rose-500 text-white py-3 rounded-xl font-bold shadow-lg hover:bg-rose-600 transition disabled:opacity-50">{loading ? 'Verificando...' : (mode === 'login' ? 'Entrar' : 'Criar')}</button>
                </form>
                <button onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setError(''); }} className="mt-6 text-sm text-rose-500 font-semibold hover:underline block w-full">
        ...
        <div className="h-screen bg-white p-6 overflow-y-auto">
            ...
            {step === 1 && (
                <div className="space-y-4">
                    ...
                    <p className="text-xs text-gray-500 bg-yellow-50 p-3 rounded">Sua Renda Líquida mensal:</p>
                    <input type="number" className="w-full border p-3 rounded-xl text-lg" placeholder="R$ 0.00" value={formData.income} onChange={e=>setFormData({...formData, income: e.target.value})} />
            ...
            {step === 2 && (
                <div className="space-y-4">
                    ...
                    <textarea placeholder="Músicas/Bandas..." className="w-full border p-2 rounded" value={formData.music} onChange={e=>setFormData({...formData, music: e.target.value})} />
                    <div className="flex gap-2 mt-4"><button onClick={()=>setStep(1)} className="flex-1 bg-gray-200 py-3 rounded-xl">Voltar</button><button onClick={()=>setStep(3)} className="flex-1 bg-rose-500 text-white py-3 rounded-xl font-bold">Próximo</button></div>
                </div>
            )}
            {step === 3 && (
                <div className="space-y-4">
                    <h2 className="font-bold text-lg text-gray-800">3. Sentimentos</h2>
                    <textarea placeholder="Como vê o relacionamento..." className="w-full border p-2 rounded h-20" value={formData.relationshipView} onChange={e=>setFormData({...formData, relationshipView: e.target.value})} />
                    ...
                    <div className="flex gap-2 mt-4"><button onClick={()=>setStep(2)} className="flex-1 bg-gray-200 py-3 rounded-xl">Voltar</button><button onClick={()=>setStep(4)} className="flex-1 bg-rose-500 text-white py-3 rounded-xl font-bold">Próximo</button></div>
                </div>
            )}
            {step === 4 && (
                <div className="space-y-4">
                    <h2 className="font-bold text-lg text-gray-800">4. Amor</h2>
                    <p className="text-sm">Como prefere receber carinho?</p>
                    <div className="space-y-2">{['Palavras de Afirmação', 'Tempo de Qualidade', 'Presentes', 'Atos de Serviço', 'Toque Físico'].map(opt => (<button key={opt} onClick={() => setFormData({...formData, loveQuizAnswers: { q1: opt }})} className={`w-full p-3 rounded border text-left ${formData.loveQuizAnswers?.q1 === opt ? 'bg-rose-100 border-rose-500 font-bold' : 'bg-white'}`}>{opt}</button>))}</div>
                    <div className="flex gap-2 mt-4"><button onClick={()=>setStep(3)} className="flex-1 bg-gray-200 py-3 rounded-xl">Voltar</button><button onClick={handleSave} className="flex-1 bg-green-500 text-white py-3 rounded-xl font-bold">Salvar</button></div>
                </div>
            )}
        </div>
    )
...
// --- Dashboard e Tabs ---
const DAILY_QUOTES=["O segredo é perdoar muito.","Amar é uma escolha diária.","Vocês são um time contra o problema.","Divida as contas proporcionalmente! 💸","Fome causa brigas. Comam antes."];
...
function Chores({ user, profileName }) {
    ...
    useEffect(() => { if (!user) return; const unsub = onSnapshot(query(collection(db, APP_COLLECTION_ID, 'chores'), orderBy('completed')), (snap) => setItems(snap.docs.map(d => ({id: d.id, ...d.data()})))); return () => unsub(); }, [user]);
    const addManualTask = async (e) => { e.preventDefault(); if (!manualTask) return; await addDoc(collection(db, APP_COLLECTION_ID, 'chores'), { title: manualTask, category: 'Personalizado', effort: parseInt(manualEffort), assignedTo: manualAssignee, completed: false, completedBy: null, createdAt: serverTimestamp() }); setManualTask(''); };
    const addSuggestedTask = async (task) => { await addDoc(collection(db, APP_COLLECTION_ID, 'chores'), { title: task.title, category: task.category, effort: task.effort, assignedTo: 'Ambos', completed: false, completedBy: null, createdAt: serverTimestamp() }); setShowSuggestions(false); setSearchQuery(''); };
    const toggle = async (item) => await updateDoc(doc(db, APP_COLLECTION_ID, 'chores', item.id), { completed: !item.completed, completedBy: !item.completed ? profileName : null });
...
function Finances({ user, profileName }) {
    ...
    useEffect(() => { if (!user) return; const f = async () => { const h = await getDoc(doc(db, APP_COLLECTION_ID, 'profiles', 'Hebert', 'info')); const j = await getDoc(doc(db, APP_COLLECTION_ID, 'profiles', 'Juan', 'info')); if (h.exists() && j.exists()) { const total = (parseFloat(h.data().income) || 0) + (parseFloat(j.data().income) || 0); if (total > 0) setIncomeRatio({ hebert: (parseFloat(h.data().income) || 0) / total, juan: (parseFloat(j.data().income) || 0) / total }); } }; f(); const u = onSnapshot(query(collection(db, APP_COLLECTION_ID, 'fixed_bills'), orderBy('dueDate')), (s) => setFixedBills(s.docs.map(d => ({id: d.id, ...d.data()})))); return () => u(); }, [user]);
    const add = async (e) => { e.preventDefault(); if (!newItem.title || !newItem.value) return; await addDoc(collection(db, APP_COLLECTION_ID, 'fixed_bills'), { ...newItem, value: parseFloat(newItem.value), dueDate: 10, isPaid: false }); setNewItem({ title: '', value: '', responsible: 'Proporcional', isRomeu: false }); };
    const toggle = async (bill) => await updateDoc(doc(db, APP_COLLECTION_ID, 'fixed_bills', bill.id), { isPaid: !bill.isPaid });
    const del = async (id) => await deleteDoc(doc(db, APP_COLLECTION_ID, 'fixed_bills', id));
    return (
        <div className="space-y-4"><div className="bg-green-50 p-4 rounded-xl border border-green-200"><h3 className="font-bold text-green-800 text-sm mb-2 flex items-center gap-2"><DollarSign size={14}/> Divisão Justa (Renda)</h3><div className="flex h-4 bg-gray-300 rounded-full overflow-hidden mb-2"><div style={{width: `${incomeRatio.hebert * 100}%`}} className="bg-indigo-500"></div><div style={{width: `${incomeRatio.juan * 100}%`}} className="bg-teal-500"></div></div><div className="flex justify-between text-xs text-green-700"><span>Hebert: {(incomeRatio.hebert * 100).toFixed(0)}%</span><span>Juan: {(incomeRatio.juan * 100).toFixed(0)}%</span></div></div><div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100"><form onSubmit={add} className="space-y-2"><input value={newItem.title} onChange={e=>setNewItem({...newItem, title: e.target.value})} placeholder="Ex: Ração, Aluguel" className="w-full border p-2 rounded text-sm" /><div className="flex gap-2"><input type="number" value={newItem.value} onChange={e=>setNewItem({...newItem, value: e.target.value})} placeholder="R$" className="w-1/3 border p-2 rounded text-sm" /><select value={newItem.responsible} onChange={e=>setNewItem({...newItem, responsible: e.target.value})} className="w-2/3 border p-2 rounded text-sm bg-white"><option value="Proporcional">Proporcional</option><option value="Hebert">Só Hebert</option><option value="Juan">Só Juan</option></select></div><div className="flex items-center gap-2 text-sm text-gray-600"><input type="checkbox" id="isRomeu" checked={newItem.isRomeu} onChange={e=>setNewItem({...newItem, isRomeu: e.target.checked})} /><label htmlFor="isRomeu" className="flex items-center gap-1 cursor-pointer select-none"><Dog size={14}/> É gasto com o Romeu?</label></div><button type="submit" className="w-full bg-green-500 text-white py-2 rounded text-sm font-bold">Salvar</button></form></div><div className="space-y-2 pb-20">{fixedBills.map(bill => { let h = 0, j = 0; if (bill.responsible === 'Proporcional') { h = bill.value * incomeRatio.hebert; j = bill.value * incomeRatio.juan; } else if (bill.responsible === 'Hebert') h = bill.value; else j = bill.value; return (<div key={bill.id} className={`p-3 rounded-lg border flex justify-between items-center ${bill.isPaid ? 'bg-green-50 border-green-200 opacity-60' : 'bg-white border-gray-200'}`}><div className="flex items-center gap-3"><button onClick={() => toggle(bill)} className={`w-5 h-5 rounded border flex items-center justify-center ${bill.isPaid ? 'bg-green-500 border-green-500 text-white' : 'border-gray-400'}`}>{bill.isPaid && <Check size={12} />}</button><div><p className="font-bold text-gray-800 text-sm flex items-center gap-1">{bill.title}{bill.isRomeu && <Dog size={12} className="text-orange-500"/>}</p><p className="text-[10px] text-gray-500">H: R${h.toFixed(0)} | J: R${j.toFixed(0)}</p></div></div><div className="text-right"><p className="font-bold text-gray-700">R$ {bill.value.toFixed(2)}</p><button onClick={() => del(bill.id)} className="text-gray-300 hover:text-red-500 text-xs">x</button></div></div>); })}</div></div>
    );
}

function Curiosities({ user, onEditProfile }) {
    const [profiles, setProfiles] = useState({});
    useEffect(() => { if (!user) return; const f = async () => { const h = await getDoc(doc(db, APP_COLLECTION_ID, 'profiles', 'Hebert', 'info')); const j = await getDoc(doc(db, APP_COLLECTION_ID, 'profiles', 'Juan', 'info')); setProfiles({ Hebert: h.exists() ? h.data() : null, Juan: j.exists() ? j.data() : null }); }; f(); }, [user]);
...
function Conflicts({ user, profileName }) {
    ...
    useEffect(() => { if (!user) return; const u = onSnapshot(query(collection(db, APP_COLLECTION_ID, 'conflicts'), orderBy('createdAt', 'desc')), (s) => setConflicts(s.docs.map(d => ({ id: d.id, ...d.data() })))); return () => u(); }, [user]);
    const send = async (e) => { e.preventDefault(); if (!formData.title) return; await addDoc(collection(db, APP_COLLECTION_ID, 'conflicts'), { ...formData, createdBy: profileName, status: 'open', createdAt: serverTimestamp() }); setFormData({ title: '', feelings: '', solution: '' }); setShowForm(false); };
...
function SharedCalendar({ user, profileName }) {
    ...
    useEffect(() => { if (!user) return; const u = onSnapshot(query(collection(db, APP_COLLECTION_ID, 'calendar'), orderBy('date')), (s) => setEvents(s.docs.map(d => ({id: d.id, ...d.data()})))); return () => u(); }, [user]);
    const add = async (e) => { e.preventDefault(); if(!newEvent.title || !newEvent.date) return; await addDoc(collection(db, APP_COLLECTION_ID, 'calendar'), newEvent); setNewEvent({ title: '', date: '', type: 'compromisso' }); };
    return (
